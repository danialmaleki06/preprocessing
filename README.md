...
`preprocessing` объединяет весь путь — от чтения сырого CSV до готовой к обучению выборки — в один последовательный, прозрачный и воспроизводимый конвейер. Каждое действие документируется отчётом, безопасные стратегии стоят по умолчанию, а защита от утечки встроена в само устройство API, а не оставлена на внимательность пользователя. Это не очередная обёртка вокруг pandas, а **дисциплина корректной предобработки, оформленная в библиотеку**.
