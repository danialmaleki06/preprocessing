...
Проект `preprocessing` объединяет весь путь — от чтения сырого CSV до готовой к обучению выборки — в один последовательный, прозрачный и воспроизводимый конвейер. Каждое действие документируется отчётом, безопасные стратегии стоят по умолчанию, а защита от утечки встроена в само устройство API, а не оставлена на внимательность пользователя.
<a id="quickstart"></a>
