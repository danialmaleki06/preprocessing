...
> Часто виновата не модель, а **утечка данных** в предобработке: тестовая выборка незаметно «подсматривает» в обучающую через общие медианы, масштабы и границы выбросов. Метрика на валидации оказывается завышенной — а реальное качество разочаровывает.
> `preprocessing` построен так, что эта ошибка **невозможна**: каждый статистический шаг честно разделён на `fit` (расчёт по train) и `apply` (применение к test и новым данным).

<a id="why"></a>

## 💡 Зачем этот проект

Предобработка данных — самая недооценённая часть машинного обучения. Дата-сайентист тратит на неё значительную долю рабочего времени, и именно здесь чаще всего возникают незаметные ошибки: перепутанный порядок операций, заполнение пропусков по всему датасету, масштабирование до разбиения. Каждая из них тихо завышает метрики и подрывает доверие к модели.
